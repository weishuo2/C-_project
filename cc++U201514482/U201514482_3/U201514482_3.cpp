...
STACK::STACK(const STACK&s):max(s.max), elems(new int[s.max])//存疑 ,栈还没有分配空间 
{
	pos = s.pos;
...
int main(int argc,char *argv[])
{
	...
	stream = fopen("U201514482_3.txt", "w" ); //文件流 
	while(num < argc)//没有处理完 
	{
		...
		else if(strcmp(argv[num],"-A") == 0)// 重新创建一个栈，然后把原来的值给它 
		{
			...
			mystack->print();
			amax = atoi(argv[num]);
			newastack=new STACK(amax);//为A建立一个栈,应该将amax改为mystack->size() 
			*newastack=*mystack;
			mystack->~STACK();
			mystack = newastack;
			num++;
